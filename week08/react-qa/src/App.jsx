...
function MyRow(props) {
  ...
  return (
    <tr>
      <td>{dayjs(e.date).format("YYYY-MM-DD")}</td>
      <td>{e.text}</td>
      <td>{e.respondent}</td>
      <td>{e.score}</td>
  ...
  return (<>
    <Table>
      ...
      <tbody>
        ...
        { showForm ? <AnswerForm addToList={addToList} closeForm={() => setShowForm(false)}></AnswerForm> : <Button onClick={() => setShowForm(true)}>Add Element</Button> }
      </>
  )
}
