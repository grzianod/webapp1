...
function FilmRow(props) {
    ...
    return (searchParams.has("edit") && parseInt(searchParams.get("edit")) === props.item.id) ? (
            ...
            <td className="text-center" style={{verticalAlign: "middle"}}>
                {
                    [...Array(5)].map((star, index) => {
                        index += 1;
                        return (rating < index) ?
                            <svg key={index+"@"+props.item.id+"E"} type="button" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#BBBBBB"
                                 className="bi bi-star-fill form null" viewBox="0 0 16 16" onClick={(event) => {
                                setRating(index);
                            }}>
                                <path
        ...
        <tr>
            ...
            <td className="text-center" style={{verticalAlign: "middle"}}>
                <Form.Check type="checkbox" id={`default-checkbox`} disabled defaultChecked={item.favorite}/></td>
            ...
            <td className="text-center" style={{verticalAlign: "middle"}}>
                {
                    [...Array(5)].map((star, index) => {
                        ...
                        return (item.rating < index) ?
                            <svg key={index} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#D3D3D3" onClick={() => {
                                setRating(index); }}
                                 className="bi bi-star-fill" viewBox="0 0 16 16">
                                <path
                                    d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                            </svg> :
                            <svg key={index} xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="#e6cb96" onClick={() => {
                                setRating(index); }}
                                 className="bi bi-star-fill" viewBox="0 0 16 16">
                                <path
                                    d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
